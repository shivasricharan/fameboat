...
export function SelectedWork(){return <section id="work" className="section section-tint"><div className="shell"><div className="section-row"><div><p className="section-label">Selected work</p><h2>Digital experiences built around real organisations.</h2></div><Link className="text-link" href="/work">View all work <ArrowRight size={16}/></Link></div><div className="work-grid"><WorkCard title="Metro TV Telugu" category="Media & publishing" text="A modern digital publishing presence designed around content discovery, live media, advertiser enquiries, and a practical Google Sheets publishing workflow." href="/work/metro-tv-telugu"/><WorkCard title="IIA Telangana" category="Professional association" text="A clearer industry-facing experience for programmes, events, information discovery, and registration pathways." href="/work/iia-telangana"/></div></div></section>}

function WorkCard({title,category,text,href}:{title:string,category:string,text:string,href:string}){return <Link href={href} className="work-card"><div className="work-visual"><div className="browser-bar"><i/><i/><i/></div><div className="work-placeholder"><span>{title}</span></div></div><p className="section-label">{category}</p><h3>{title}</h3><p>{text}</p><span className="text-link">View case study <ArrowRight size={15}/></span></Link>}

export function FounderSection(){return <section className="section"><div className="shell founder-grid"><div className="founder-mark">19<span>+</span></div><div><p className="section-label">Why Fameboat</p><h2>Built from business understanding, not website templates.</h2><p>Fameboat is led by a professional with more than 19 years of experience across marketing, branding, media, digital touchpoints, customer journeys, technology, and business implementation.</p><p>The work begins with how a customer understands and experiences the business—not with a theme, template, or preferred technology.</p><Link href="/about" className="text-link">About Fameboat <ArrowRight size={16}/></Link></div></div></section>}
